...
void LeStringCadastro(char CadastroGenerico[]){ // função utilizada para entrada de dados em strings de tamanho igual e que não aceitam numeros, como nome, rua e bairro.
	...
	while(continua){
		...
		if(!isalpha(CadastroGenerico[0])){
		    continua = true;
		}
		if(continua){
			printf("\nNumeros e caracteres diferentes de letras nessa area nao sao permitidos. Por favor, digite novamente o que foi solicitado: ");
		}
	}
}
...
void LeCpfClientes(char client_cpf[], FILE *arqv){ //funcao que le, pela entrada de teclado, o cpf do cliente.
	...
	do{
        ...
        getchar();
        if(ValidaCpfs(client_cpf, arqv)){
        	valido = true;
		}
		else{
			valido = false;
			printf("\nInvalido. Ou o cpf informado e invalido ou ele ja esta cadastrado no nosso sistema");
		}
...
int NumCasa(){ //funcao que lê pelo teclado o numero de residencia do cliente
	int num; 
	do{
		printf("\nInforme o numero da casa: (Caso a sua residencia nao possua um numero, informe 0): ");
		scanf("%d", &num);
	}while(num < 0);
	return num;
}
...
void LeDatasGenerico(char DDMMYYYY[]){ //Função genérica para ler valores relacionados à data.
	...
	scanf("%d", &ano);
	while (ano < 1 || ano > 2023) {
	    printf("\nInvalido! Informe o ano: ");
	    scanf("%d", &ano);
	}
	printf("\nInforme o mes: ");
	scanf("%d", &mes);
	...
	switch(mes){
		...
		case 2: //mês fevereiro tem duas opções de dias, sendo considerado um ano bissexto ou nao.
			if (ano % 400 == 0 || (ano % 4 == 0 && ano % 100 != 0)) { //Condição de ano bissexto
		        do{
		        	printf("\nLegal, o ano e bissexto. Informe o dia (entre 1 e 29): ");
		        	scanf("%d", &dia);
				}while (dia > 29 || dia < 1);
		    } 
			else { //condição !ano bissexto, (negado hehe)
		        do{
		        	printf("\nInforme o dia (entre 1 e 28): ");
		        	scanf("%d", &dia);
				}while (dia > 28 || dia < 1);
		    }
		break;
...
bool ValidaAnos(char AnoC[], char AnoN[]){ //função para validação de ano de compra, uma vez que o ano de compra não pode ser inferior ou igual a o ano de nascimento.
    int AnoCompra, AnoNascimento;
    bool AnosInvalidos = false;
    AnoCompra = atoi(AnoC + 6); //A sintexe do ATOI permite escolher de que posição de uma string desejamos começar a  atribuir. Nesse caso, por se tratar de DD/MM/YYYY, usa-se a partir da posição 6 do vetor
    AnoNascimento = atoi(AnoN + 6);
    if (AnoCompra <= AnoNascimento){
        AnosInvalidos = true;
    }
    return AnosInvalidos;
}
void CadastraCliente(dados *cliente, FILE *arquivo_db){
	...
	printf("\nAbaixo, informe a sua data de nascimento");
	LeDatasGenerico(cliente->data_nasc);
	do{
		printf("\nAbaixo, Informe a data da sua ultima compra. Obs: nao e possivel uma data de compra anterior a data de nascimento");
		LeDatasGenerico(cliente->data_compra);
	}while(ValidaAnos(cliente->data_compra, cliente->data_nasc));
