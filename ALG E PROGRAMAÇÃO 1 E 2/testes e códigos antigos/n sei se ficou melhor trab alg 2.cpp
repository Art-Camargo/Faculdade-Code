...
void CadastraCliente(dados *cliente, FILE *arquivo_db){
	...
	while(RetornaAnos(cliente->data_nasc, 6) > 2022){ //Nao e permitirdo cadastros de pessoas com menos de 1,5 ano.
...
void valorMAXac(FILE *arquivoTOTAL) {
	char *token, *al;
	float max = 0.0, atual;
	int LineMax = 0, h = 0, k = 0;
	al = (char*) malloc(200*sizeof(char));
	if(al != NULL){
		while(fgets(al, 200, arquivoTOTAL) != NULL){
			token = strtok(al, ";");
			for(h = 0; h < 2; h++){
				if(h == 1){
