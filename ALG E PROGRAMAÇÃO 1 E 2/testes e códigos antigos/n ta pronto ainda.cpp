...
typedef struct informacoes{
    char nome[30], cpf[12], rua[30], bairro[30], data_compra[9], data_nasc[9], valor_acumulado[20];
    int num_rua; //numero da rua //nomedo bairro //valor acumulado em compras
}dados;
void print_menu(){
	printf("\n1 - CADASTRAR CLIENTE");
...
bool GENERIC_SEARCH(char generico_char[], FILE *arquivoDOSclientes){
	...
	if(linha_arquivo == NULL){
    	...
    	while(fgets(linha_arquivo, 250, arquivoDOSclientes) != NULL){
...
int main(){
	...
	dados db_clients;
	int opcao, auxx;
	char bairros_busca[30], data_busca[10], url[] = "Cadastro-clientes.txt";
	float max;
	do{
		print_menu();
		opcao = MenuOption();
		...
		switch(opcao){
			case 1: 
				...
				if(arquivo){
					...
					fprintf(arquivo, "%s, cpf: %s, rua: %s, num: %d*, bairro: %s., %s, %s ;%s\n", 
